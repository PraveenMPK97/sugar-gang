...
const SearchPatients: React.FC = () => {
    ...
    const handleDeleteClick = async (patientId: number) => {
        try {
            await fetch(`/api/deletePatient?patientId=${patientId}`, { method: 'DELETE' });
                        ...
                        <TableBody>
                            {patients.map((patient) => (
                                <React.Fragment key={patient.patientId}>
                                    <TableRow>
                                        <TableCell>
                                            <IconButton size="small" onClick={() => handleExpandClick(patient.patientId)}>
                                                {expandedPatientId === patient.patientId ? <ExpandLessIcon /> : <ExpandMoreIcon />}
                                            </IconButton>
                                        </TableCell>
                                        ...
                                        <TableCell>
                                            ...
                                            <IconButton onClick={() => handleDeleteClick(patient.patientId)}>
                                                <DeleteIcon />
                                            </IconButton>
                                        </TableCell>
                                    </TableRow>
                                        ...
                                        <TableCell colSpan={8} style={{ paddingBottom: 0, paddingTop: 0 }}>
                                            <Collapse in={expandedPatientId === patient.patientId} timeout="auto" unmountOnExit>
                                                <Table size="small" aria-label="grbs-investigations">
                                                    ...
                                                    <TableBody>
                                                        {patient.patientDetails.slice(0, 3).map((detail) => (
